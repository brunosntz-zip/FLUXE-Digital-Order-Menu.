...
class Comanda(models.Model):
    ...
    encerrada_por = models.ForeignKey('Usuario', on_delete=models.PROTECT, db_column='encerrada_por', blank=True, null=True)
...
class Produto(models.Model):
    ...
    # RELACIONAMENTO CORRIGIDO COM VÍRGULAS 👇
    sugestoes = models.ManyToManyField(
        'self',
        blank=True,
        symmetrical=False,
        verbose_name="Combina com (Cross-selling)"
    )
...
class Pedido(models.Model):
    ...
    def __str__(self):
        return f"Pedido {self.id} (Comanda: {self.comanda.id})"
...
class PedidoStatusHistorico(models.Model):
    ...
    pedido = models.ForeignKey(Pedido, on_delete=models.PROTECT)
    status = models.CharField(max_length=15)
    alterado_por = models.ForeignKey('Usuario', on_delete=models.PROTECT, db_column='alterado_por', blank=True, null=True)
